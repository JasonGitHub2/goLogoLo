...
public class AddTextDialog extends Stage {
    ...
    TextField addTextComponentTextField = new TextField();   
    ...
    public AddTextDialog(GoLogoLo initApp) {
    ...
    private void initDialog() {
        ...
        initGridNode(nameLabel,             LOGO_NAME_LABEL,                   CLASS_LOGO_DIALOG_PROMPT,       0, 1, 1, 1, true);
        initGridNode(nameTextField,         null,                              CLASS_LOGO_DIALOG_TEXT_FIELD,   1, 1, 1, 1, false);
        initGridNode(addTextLabel,             LOGO_TEXT_LABEL,                CLASS_LOGO_DIALOG_PROMPT,       0, 2, 1, 1, true);
        initGridNode(addTextComponentTextField,         null,                  CLASS_LOGO_DIALOG_TEXT_FIELD,   1, 2, 1, 1, false);
        initGridNode(okCancelPane,          null,                              CLASS_LOGO_DIALOG_PANE,         0, 3, 3, 1, false);

        okButton = new Button();
        cancelButton = new Button();
       app.getGUIModule().addGUINode(LOGO_ADD_TEXT_DIALOG_OK_BUTTON, okButton);
       app.getGUIModule().addGUINode(LOGO_ADD_TEXT_DIALOG_CANCEL_BUTTON, cancelButton);
       ...
       okCancelPane.setAlignment(Pos.CENTER);
        ...
        AppLanguageModule languageSettings = app.getLanguageModule();
        languageSettings.addLabeledControlProperty(LOGO_DIALOG_OK_BUTTON + "_TEXT",    okButton.textProperty());
        languageSettings.addLabeledControlProperty(LOGO_DIALOG_CANCEL_BUTTON + "_TEXT",    cancelButton.textProperty());
       
        // AND SETUP THE EVENT HANDLERS
        nameTextField.setOnAction(e->{
            processCompleteWork();
        });
          addTextComponentTextField.setOnAction(e->{
            processCompleteWork();
        });
    ...
    public void showAddTextDialog(){
        ...
        addTextComponentTextField.setText("");
     ...
     private void makeNewItem() {
        ...
        String text = addTextComponentTextField.getText();
        newItem = new LogoPrototype(name,"Text",text);
        this.hide();
    }
    
    public void processCompleteWork(){
        String name = nameTextField.getText();
        String thisText = addTextComponentTextField.getText();
        LogoData data=(LogoData)app.getDataComponent();
         if (data.isValidLogoData(name)&&data.isValidLogoData(thisText)) {
         this.makeNewItem();
         }
         else{
    ...
    public LogoPrototype getNewItem() {
        return newItem;
    }
